...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre==""){
		...
		header('location:/admin/nuevo/filtro/tamano');
		exit;
	}else{
        
		$caractCount	=	$db->getQueryCount('carTamano','id');
		if($caractCount[0]['total']<10000){
			$data	=	array(
							'nombre'=>($nombre),
                            
						);
			$insert	=	$db->insert('carTamano',$data);
			if($insert){
                setcookie("msg","catok",time() + 2, "/");
				header('location:/admin/nuevo/filtro/tamano');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/admin/nuevo/filtro/tamano');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/admin/nuevo/filtro/tamano'); //limite
			exit;
		}
	}
}
    ...
    <div class="main-wrapper main-wrapper-1">
      ...
      <div class="main-content">
        ...
        <ul class="breadcrumb breadcrumb-style ">
            ...
            <li class="breadcrumb-item active">Agregar un nuevo tamaño</li>
        </ul>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $categoriaData = $db->getAllRecords('carTamano','*','ORDER BY id DESC');
                                      ...
                                      $y	=	'';
                                          foreach($categoriaData as $categoria){
                                              ...
                                              <tr>
                                                  ...
                                                  <td class="text-rigth">
                                                      <a href="/admin/editar/carTamano?editId=<?php echo $categoria['id']; ?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a>
                                                      <a href="/admin/borrar/carTamano?delId=<?php echo $categoria['id']; ?>" onClick="return confirm('Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                  </td>
                                              </tr>
