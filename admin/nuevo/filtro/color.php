...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre==""){
		...
		header('location:/admin/nuevo/filtro/color');
		exit;
	}else{
        
		$caractCount	=	$db->getQueryCount('carColor','id');
		if($caractCount[0]['total']<10000){
			$data	=	array(
							'nombre'=>($nombre),
							'exadecimal'=>($exadecimal),
                            
						);
			$insert	=	$db->insert('carColor',$data);
			if($insert){
                setcookie("msg","catok",time() + 2, "/");
				header('location:/admin/nuevo/filtro/color');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/admin/nuevo/filtro/color');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/admin/nuevo/filtro/color'); //limite
			exit;
		}
	}
}
    ...
    <div class="main-wrapper main-wrapper-1">
      ...
      <div class="main-content">
        ...
        <ul class="breadcrumb breadcrumb-style ">
            ...
            <li class="breadcrumb-item active">Agregar un color</li>
        </ul>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label>Color</label>
                                    <input name="exadecimal" type="color" class="form-control">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $categoriaData = $db->getAllRecords('carColor','*','ORDER BY id DESC');
                                      ...
                                      $y	=	'';
                                          foreach($categoriaData as $categoria){
                                              ...
                                              <tr>
                                                  ...
                                                  <td><h1 class="badge" style="background: <?php echo $categoria['exadecimal']; ?>;"> </h1></td>
                                                  <td class="text-rigth">
                                                      <a href="/admin/editar/carColor?editId=<?php echo $categoria['id']; ?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a>
                                                      <a href="/admin/borrar/carColor?delId=<?php echo $categoria['id']; ?>" onClick="return confirm('Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                  </td>
                                              </tr>
