...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre==""){
		...
		$caractCount	=	$db->getQueryCount('carForma','id');
		if($caractCount[0]['total']<10000){
			$data	=	array(
							'nombre'=>($nombre),
                            
						);
			$insert	=	$db->insert('carForma',$data);
			if($insert){
                setcookie("msg","catok",time() + 2, "/");
				header('location:/admin/nuevo/filtro/forma');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/admin/nuevo/filtro/forma');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/admin/nuevo/filtro/forma'); //limite
			exit;
		}
	}
}
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $categoriaData = $db->getAllRecords('carForma','*','ORDER BY id DESC');
                                      ...
                                      $y	=	'';
                                          foreach($categoriaData as $categoria){
                                              ...
                                              <tr>
                                                  ...
                                                  <td class="text-rigth">
                                                      <a href="/admin/editar/carForma?editId=<?php echo $categoria['id']; ?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a>
                                                      <a href="/admin/borrar/carForma?delId=<?php echo $categoria['id']; ?>" onClick="return confirm('Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                  </td>
                                              </tr>
