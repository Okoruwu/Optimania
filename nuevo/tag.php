...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre==""){
		...
		header('location:/admin/nuevo/tag');
		exit;
	}else{
        
		$caractCount	=	$db->getQueryCount('tags','id');
		if($caractCount[0]['total']<10000){
			$data	=	array(
							'nombre'=>($nombre),
                            
						);
			$insert	=	$db->insert('tags',$data);
			if($insert){
                setcookie("msg","tagok",time() + 2, "/");
				header('location:/admin/nuevo/tag');//exito
				exit;
			}else{
                setcookie("msg","ups",time() + 2, "/");
				header('location:/admin/nuevo/tag');//sin cambios
				exit;
			}
		} else{
            setcookie("msg","lim",time() + 2, "/");
			header('location:/admin/nuevo/tag'); //limite
			exit;
		}
	}
}
...
<head>
  ...
  <title>Optimania - Nueva - Tag</title>
   <!-- General CSS Files -->
    ...
    <div class="main-wrapper main-wrapper-1">
      ...
      <div class="main-content">
        ...
        <ul class="breadcrumb breadcrumb-style ">
            ...
            <li class="breadcrumb-item active">Agregar una nuevo tag</li>
        </ul>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                <?php $tagData = $db->getAllRecords('tags','*','ORDER BY id DESC');
                                  if (count($tagData)>0){
                                      $y	=	'';
                                          foreach($tagData as $tag){
                                              ...
                                              <tr>
                                                  ...
                                                  <td class="text-rigth">
                                                      <a href="/admin/editar/tag?editId=<?php echo $tag['id']; ?>" class="btn btn-icon btn-primary"><i class="far fa-edit"></i></a>
                                                      <a href="/admin/borrar/tag?delId=<?php echo $tag['id']; ?>" onClick="return confirm('Estás seguro? Esto no se puede deshacer');" class="btn btn-icon btn-danger"><i class="fa fa-trash"></i></a>
                                                  </td>
                                              </tr>
