...
<body>
    <div class="ps-page">
        ...
        <?php require_once($_SERVER["DOCUMENT_ROOT"] . "/modulos/vista.php"); ?>

        <div class="ps-portfolio">
            <div class="container">
                <div class="ps-portfolio__content">
                    <h3 style="color: gray; font-size: 30px" class="text-center"><a href="/lentes/vista/tamano">TAMAÑO</a> | <a style="color: #f18500;" href="/lentes/vista/color">COLOR</a> | <a href="/lentes/vista/material">METRIAL</a> | <a href="/lentes/vista/forma">FORMA</a> | <a href="/lentes/vista/marca">MARCA</a></h3>
                    <br>
                    <div class="ps-portfolio__tabs">
                        <ul class="nav nav-tabs text-center justify-content-center" id="productContentTabs" role="tablist">

                            <?php
                            $catData = $db->getAllRecords('carColor', '*', ' ORDER BY nombre ASC');
                            if (count($catData) > 0) {
                                $y    =    '';
                                    ...
                                    <div class="tab-pane fade <?php echo $active; ?>" id="portfolio-all-<?php echo ($categoria['id']); ?>" role="tabpanel" aria-labelledby="portfolio-all-tab">
                                        <div class="ps-categogy--grid ps-categogy--detail">
                                            <div class="row">
                                                ...
                                                $productosData = $db->getAllRecords('productos', '*', 'AND categoria="3" ORDER BY nombre ASC');

                                                if (count($productosData) > 0) {

                                                    foreach ($productosData as $producto) {
                                                        //REVISAMOS QUE ESTOS LENTES ESTEN EN ESTA CARACTERISTICA
                                                        $productoEnCar = $db->getAllRecords('carColorEnProducto', '*', 'AND producto="' . $producto['id'] . '" ORDER BY id ASC');
