...
if (isset($_REQUEST['editId']) and $_REQUEST['editId']!=""){
	$caracteristica  =  $db->getAllRecords('caracteristicas','*',' AND id="'.$_REQUEST['editId'].'"','LIMIT 1');
    
        if (empty($caracteristica)) { //SI NO EXISTE ES QUE NO HAY UN ID VALIDO Y REDIRECCIONAMOS Y LANZAMOS ERROR
            setcookie("msg","ups",time() + 2, "/");
            header('location:/admin/nuevo/caracteristica');
            exit;
        } else {$caracteristica  =  $caracteristica [0];}
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	if($nombre==""){
        ...
        if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	       ...
	       $update	=	$db->update('caracteristicas',$data,array('id'=>($_REQUEST['editId'])));
    
            if($update){
                setcookie("msg","carup",time() + 2, "/");
                header('location:/admin/nuevo/caracteristica'); //Exito en el cmabio
                exit;}
                else{
                    setcookie("msg","ups",time() + 2, "/");
            ...
            <div class="col-md-12 col-lg-12 col-xl-8">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4>Estás editando: <?php echo ($caracteristica['nombre']);?></h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-3">
                                <div class="form-group">
                                    <label>Nombre</label>
                                    <input name="nombre" value="<?php echo ($caracteristica['nombre']);?>" type="text" class="form-control">
                                </div>
                            </div>
                            <div class="col-12">
